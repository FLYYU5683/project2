...
import {useOrb,countSwing,resetPlayData,replayAll,resetCameraAngle,fovX,inHoleBreak,moveMode,aimModeChange,setOther,openMap,openMapFalse,hitting,hittingFalse,countSwingReset,ballMoveFalse} from './touchEvent.js'
...
function buildCamAndSen(){
  ...
  cameraForFoot1 = new THREE.OrthographicCamera(-150, 150, 300, -300, -10, 1000);
  cameraForFoot2 = new THREE.OrthographicCamera(-250, 250, 300, -300, -10, 1000);
  cameraForFoot3 = new THREE.OrthographicCamera(-150, 150, 300, -300, -10, 1000);
...
function buildHUD(){
	...
	let purpleArrowTexture = textureloader.load("https://i.imgur.com/8dUcgUV.png")
	let purpleArrowMaterial = new THREE.MeshBasicMaterial({
		opacity: 1,
		alphaTest:0.5,
		transparent: true,
		depthTest: false,
		depthWrite: false,
		map: purpleArrowTexture});
	  
	let purpleArrow = new THREE.Mesh(new THREE.PlaneGeometry(5, 2),purpleArrowMaterial)
	purpleArrow.position.set(4.5,0,0);
	ballMoveControl.add(purpleArrow.clone());
	purpleArrow.rotation.z = Math.PI;
	
	purpleArrow.position.set(-4.5,0,0);
	ballMoveControl.add(purpleArrow.clone());
	
	purpleArrow.scale.set(0.5,2,1);
	purpleArrow.position.set(0,-2.25,0);
	purpleArrow.rotation.z = -Math.PI/2;
	ballMoveControl.add(purpleArrow.clone());
...
function HUDPress(){
	...
	else if(Math.abs(touch.x) <= 0.4 && Math.abs(touch.y) <= 0.4){
	    if(start === false){
			start = true;
			startButton.visible = false
			switchCamera = 1;
		...
		if(isOver === false){
			...
			if(mode === 1){
				...
				/// select level or move ball
					...
					if(between(touch.x,-0.5,-0.9) && between(touch.y,0.66,0.56)){
						balls[0].start();
						balls[1].start();
						steve.start();
						modeChose = true;
					...
					if(between(touch.x,-0.1,-0.5) && between(touch.y,0.66,0.56)){///////show steve
						SteveShow=!SteveShow;
						if(SteveShow==true){
							SteveShowButton.visible =true;
							SteveHideButton.visible =false;
